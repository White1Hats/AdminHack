...
echo -e "\t${colors[rand1]}   \_/\_/   |_| |_| |_|  \__|  \___|   |_| |_|  \__ _|  \__| |___/"
	...
	echo -e "                 \t${bd}${g}│${w} Codigo    ${b}:${w} Bash, python                   ${g}${n}"
...
clear
banner
echo -ne "      \t${c}[${w}>${c}] ${w}Coloque o Site ${g}:${n} "
read web
if [[ -z $web ]]; then
	printf "\n"
	echo -e "${b}[${r}!${b}]${w} Erro! o site é invalido"
	exit 0
fi
echo -ne "      \t${c}[${w}>${c}] ${w}coloque seu wordlist ${g}(${w}Default${g}:${w} wordlist.txt${g}) ${g}:${n} "
read wordlist
web=$( echo ${web} | cut -d '/' -f 3 )
wordlist=${wordlist:-wordlist.txt}
if ! [[ -e $wordlist ]]; then
	printf "\n"
	echo -e "${b}[${r}!${b}]${w} Lista não encontrada !!"
	exit 0
fi
